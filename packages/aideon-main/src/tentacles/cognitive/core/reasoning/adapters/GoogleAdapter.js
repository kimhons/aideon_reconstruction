/**
 * @fileoverview Adapter for Google models, providing integration with Gemini models
 * across all reasoning strategies. This adapter implements the standardized model
 * adapter interface for the Reasoning Engine.
 * 
 * The GoogleAdapter provides specialized integration with Google's Gemini models,
...
/**
 * Adapter for Google models, providing integration with Gemini models
 * across all reasoning strategies.
 */
class GoogleAdapter extends EventEmitter {
  /**
   * Constructor for GoogleAdapter.
  ...
  constructor(options) {
    ...
    if (!options) throw new Error('Options are required for GoogleAdapter');
    if (!options.logger) throw new Error('Logger is required for GoogleAdapter');
    if (!options.configService) throw new Error('ConfigService is required for GoogleAdapter');
    if (!options.performanceMonitor) throw new Error('PerformanceMonitor is required for GoogleAdapter');
    if (!options.securityManager) throw new Error('SecurityManager is required for GoogleAdapter');
    if (!options.credentialManager) throw new Error('CredentialManager is required for GoogleAdapter');
    if (!options.cacheManager) throw new Error('CacheManager is required for GoogleAdapter');
    ...
    this.logger.info('GoogleAdapter created');
  }
  ...
  async initialize() {
    try {
      this.logger.info('Initializing GoogleAdapter');
      this.performanceMonitor.startTimer('google.initialize');
      
      // Load configuration
      this.modelConfig = await this.configService.getModelConfig('google');
      if (!this.modelConfig) {
        throw new Error('Failed to load configuration for Google models');
      }
      ...
      // Initialize cache
      await this.cacheManager.initializeCache('google', {
      ...
      this.performanceMonitor.stopTimer('google.initialize');
      this.logger.info('GoogleAdapter initialized successfully');
      
      return true;
    } catch (error) {
      this.performanceMonitor.stopTimer('google.initialize');
      this.logger.error(`Failed to initialize GoogleAdapter: ${error.message}`, { error });
      this.initialized = false;
      this.available = false;
  ...
  async _initializeApiClient() {
    this.logger.info('Initializing Google API client');
    ...
    try {
      ...
      if (!credentials) {
        throw new Error('No valid credentials available for Google API');
      }
      
      // Create API client
      const { GoogleAIClient } = require(this.modelConfig.apiModule || './google_ai_client');
      
      const apiClient = new GoogleAIClient({
        apiEndpoint: this.modelConfig.apiEndpoint,
        apiVersion: this.modelConfig.apiVersion || 'v1',
        timeout: this.modelConfig.timeout || 60000,
        logger: this.logger,
        projectId: this.modelConfig.projectId
      });
      ...
      this.logger.info('Google API client initialized successfully');
    } catch (error) {
      this.logger.error(`Failed to initialize Google API client: ${error.message}`, { error });
      throw error;
    }
  }
    ...
    setInterval(async () => {
      try {
        ...
        if (this.available !== isAvailable) {
          ...
          this.logger.info(`Google API availability changed to: ${isAvailable}`);
        }
      } catch (error) {
        this.logger.error(`Health check failed for Google API: ${error.message}`, { error });
      }
    }, healthCheckInterval);
  }
  ...
  getAdapterInfo() {
    return {
      ...
      version: '1.0.0',
      provider: 'Google',
      description: 'Adapter for Google models, providing integration with Gemini models across all reasoning strategies',
  ...
  getCapabilities() {
    return {
      maxTokens: this.modelConfig?.contextSize || 32768, // Gemini 1.5 Pro default
  ...
  async isAvailable() {
    ...
    try {
      this.performanceMonitor.startTimer('google.isAvailable');
      ...
      this.performanceMonitor.stopTimer('google.isAvailable');
      return isAvailable;
    } catch (error) {
      this.performanceMonitor.stopTimer('google.isAvailable');
      this.logger.error(`Error checking Google API availability: ${error.message}`, { error });
      return false;
    }
  }
  ...
  async generateCompletion(options) {
    if (!this.initialized) {
      throw new Error('GoogleAdapter is not initialized');
    }
    ...
    try {
      this.performanceMonitor.startTimer('google.generateCompletion');
      ...
      const cachedResult = await this.cacheManager.get('google', cacheKey);
      
      if (cachedResult) {
        this.metrics.cacheHits++;
        this.performanceMonitor.stopTimer('google.generateCompletion');
      ...
      // Cache result
      await this.cacheManager.set('google', cacheKey, result);
      ...
      this.performanceMonitor.stopTimer('google.generateCompletion');
      this.logger.error(`Error generating completion with Google API: ${error.message}`, { error, options });
  ...
  async generateEmbeddings(options) {
    ...
    try {
      this.performanceMonitor.startTimer('google.generateEmbeddings');
      this.metrics.totalRequests++;
      ...
      // Determine model to use
      const model = options.model || this.modelConfig.embeddingModel || 'embedding-001';
      ...
      if (cachedResult) {
        this.metrics.cacheHits++;
        this.performanceMonitor.stopTimer('google.generateEmbeddings');
      ...
      this.metrics.failedRequests++;
      this.performanceMonitor.stopTimer('google.generateEmbeddings');
      this.logger.error(`Error generating embeddings with Google API: ${error.message}`, { error, options });
  ...
  /**
   * Process a reasoning task using Google models.
  ...
  async processTask(task) {
    ...
    try {
      this.performanceMonitor.startTimer(`google.processTask.${task.type}`);
      this.metrics.totalRequests++;
      ...
      if (cachedResult) {
        this.metrics.cacheHits++;
        this.performanceMonitor.stopTimer(`google.processTask.${task.type}`);
      ...
      this.metrics.failedRequests++;
      this.performanceMonitor.stopTimer(`google.processTask.${task.type}`);
      this.logger.error(`Error processing task with Google API: ${error.message}`, { error, task });
...
`;
  ...
  async shutdown() {
    try {
      this.logger.info('Shutting down GoogleAdapter');
      ...
      this.logger.info('GoogleAdapter shut down successfully');
      return true;
    } catch (error) {
      this.logger.error(`Failed to shut down GoogleAdapter: ${error.message}`, { error });
      return false;
    }
  }
  ...
  async _getCredentials(userId) {
    try {
      ...
      if (userId) {
        const userCredentials = await this.credentialManager.getUserCredentials(userId, 'google');
      ...
      const globalCredentials = await this.credentialManager.getGlobalCredentials('google');
      if (globalCredentials && globalCredentials.apiKey) {
        return globalCredentials;
      }
      
      this.logger.warn('No valid credentials found for Google API');
      return null;
    } catch (error) {
      this.logger.error(`Error getting credentials for Google API: ${error.message}`, { error });
      return null;
    }
  }
  ...
  _estimateCost(model, usage) {
    try {
      ...
      const pricing = {
        ...
        'gemini-1.5-flash': {
          input: 0.0000035, // $3.50 per million input tokens
          output: 0.0000105 // $10.50 per million output tokens
        },
        'gemini-1.0-pro': {
