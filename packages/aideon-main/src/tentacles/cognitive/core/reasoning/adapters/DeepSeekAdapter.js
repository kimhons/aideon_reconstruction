/**
 * @fileoverview Adapter for DeepSeek models, providing integration with DeepSeek's
 * advanced language models across all reasoning strategies. This adapter implements 
 * the standardized model adapter interface for the Reasoning Engine.
 * 
 * The DeepSeekAdapter provides specialized integration with DeepSeek's models,
...
/**
 * Adapter for DeepSeek models, providing integration with DeepSeek's
 * language models across all reasoning strategies.
 */
class DeepSeekAdapter extends EventEmitter {
  /**
   * Constructor for DeepSeekAdapter.
  ...
  constructor(options) {
    ...
    if (!options) throw new Error('Options are required for DeepSeekAdapter');
    if (!options.logger) throw new Error('Logger is required for DeepSeekAdapter');
    if (!options.configService) throw new Error('ConfigService is required for DeepSeekAdapter');
    if (!options.performanceMonitor) throw new Error('PerformanceMonitor is required for DeepSeekAdapter');
    if (!options.securityManager) throw new Error('SecurityManager is required for DeepSeekAdapter');
    if (!options.credentialManager) throw new Error('CredentialManager is required for DeepSeekAdapter');
    if (!options.cacheManager) throw new Error('CacheManager is required for DeepSeekAdapter');
    ...
    this.logger.info('DeepSeekAdapter created');
  }
  ...
  async initialize() {
    try {
      this.logger.info('Initializing DeepSeekAdapter');
      this.performanceMonitor.startTimer('deepseek.initialize');
      
      // Load configuration
      this.modelConfig = await this.configService.getModelConfig('deepseek');
      if (!this.modelConfig) {
        throw new Error('Failed to load configuration for DeepSeek models');
      }
      ...
      // Initialize cache
      await this.cacheManager.initializeCache('deepseek', {
      ...
      this.performanceMonitor.stopTimer('deepseek.initialize');
      this.logger.info('DeepSeekAdapter initialized successfully');
      
      return true;
    } catch (error) {
      this.performanceMonitor.stopTimer('deepseek.initialize');
      this.logger.error(`Failed to initialize DeepSeekAdapter: ${error.message}`, { error });
      this.initialized = false;
      this.available = false;
  ...
  async _initializeApiClient() {
    this.logger.info('Initializing DeepSeek API client');
    ...
    try {
      ...
      if (!credentials) {
        throw new Error('No valid credentials available for DeepSeek API');
      }
      
      // Create API client
      const { DeepSeekClient } = require(this.modelConfig.apiModule || './deepseek_client');
      ...
      this.logger.error(`Failed to initialize DeepSeek API client: ${error.message}`, { error });
      throw error;
    }
  }
    ...
    setInterval(async () => {
      try {
        ...
        if (this.available !== isAvailable) {
          ...
          this.logger.info(`DeepSeek API availability changed to: ${isAvailable}`);
        }
      } catch (error) {
        this.logger.error(`Health check failed for DeepSeek API: ${error.message}`, { error });
      }
    }, healthCheckInterval);
  }
  ...
  getAdapterInfo() {
    return {
      ...
      version: '1.0.0',
      provider: 'DeepSeek',
      description: 'Adapter for DeepSeek models, providing integration with DeepSeek\'s language models across all reasoning strategies',
  ...
  getCapabilities() {
    return {
      maxTokens: this.modelConfig?.contextSize || 32768, // DeepSeek-V2 default
      ...
      multimodal: this.modelConfig?.multimodal || false,
      supportsVision: this.modelConfig?.supportsVision || false,
      supportsFunctionCalling: this.modelConfig?.supportsFunctionCalling || true
    };
  }
  ...
  async isAvailable() {
    ...
    try {
      this.performanceMonitor.startTimer('deepseek.isAvailable');
      ...
      this.performanceMonitor.stopTimer('deepseek.isAvailable');
      return isAvailable;
    } catch (error) {
      this.performanceMonitor.stopTimer('deepseek.isAvailable');
      this.logger.error(`Error checking DeepSeek API availability: ${error.message}`, { error });
      return false;
    }
  }
  ...
  async generateCompletion(options) {
    if (!this.initialized) {
      throw new Error('DeepSeekAdapter is not initialized');
    }
    ...
    try {
      this.performanceMonitor.startTimer('deepseek.generateCompletion');
      ...
      const cachedResult = await this.cacheManager.get('deepseek', cacheKey);
      
      if (cachedResult) {
        this.metrics.cacheHits++;
        this.performanceMonitor.stopTimer('deepseek.generateCompletion');
      ...
      // Cache result
      await this.cacheManager.set('deepseek', cacheKey, result);
      ...
      this.performanceMonitor.stopTimer('deepseek.generateCompletion');
      this.logger.error(`Error generating completion with DeepSeek API: ${error.message}`, { error, options });
  ...
  async generateEmbeddings(options) {
    ...
    try {
      this.performanceMonitor.startTimer('deepseek.generateEmbeddings');
      this.metrics.totalRequests++;
      ...
      // Determine model to use
      const model = options.model || this.modelConfig.embeddingModel || 'embedding-v1';
      ...
      if (cachedResult) {
        this.metrics.cacheHits++;
        this.performanceMonitor.stopTimer('deepseek.generateEmbeddings');
      ...
      this.metrics.failedRequests++;
      this.performanceMonitor.stopTimer('deepseek.generateEmbeddings');
      this.logger.error(`Error generating embeddings with DeepSeek API: ${error.message}`, { error, options });
  ...
  /**
   * Process a reasoning task using DeepSeek models.
  ...
  async processTask(task) {
    ...
    try {
      this.performanceMonitor.startTimer(`deepseek.processTask.${task.type}`);
      this.metrics.totalRequests++;
      ...
      if (cachedResult) {
        this.metrics.cacheHits++;
        this.performanceMonitor.stopTimer(`deepseek.processTask.${task.type}`);
      ...
      this.metrics.failedRequests++;
      this.performanceMonitor.stopTimer(`deepseek.processTask.${task.type}`);
      this.logger.error(`Error processing task with DeepSeek API: ${error.message}`, { error, task });
...
`;
  ...
  async shutdown() {
    try {
      this.logger.info('Shutting down DeepSeekAdapter');
      ...
      this.logger.info('DeepSeekAdapter shut down successfully');
      return true;
    } catch (error) {
      this.logger.error(`Failed to shut down DeepSeekAdapter: ${error.message}`, { error });
      return false;
    }
  }
  ...
  async _getCredentials(userId) {
    try {
      ...
      if (userId) {
        const userCredentials = await this.credentialManager.getUserCredentials(userId, 'deepseek');
      ...
      const globalCredentials = await this.credentialManager.getGlobalCredentials('deepseek');
      if (globalCredentials && globalCredentials.apiKey) {
        return globalCredentials;
      }
      
      this.logger.warn('No valid credentials found for DeepSeek API');
      return null;
    } catch (error) {
      this.logger.error(`Error getting credentials for DeepSeek API: ${error.message}`, { error });
      return null;
    }
  }
  ...
  _estimateCost(model, usage) {
    try {
      ...
      const pricing = {
        ...
        'deepseek-coder': {
          input: 0.000006, // $6 per million input tokens
          output: 0.000018 // $18 per million output tokens
        },
        // Embedding models
        'embedding-v1': {
      ...
      // Default to DeepSeek-V2 pricing if model not found
      const modelPricing = pricing[model] || pricing['deepseek-v2'];
