/**
 * @fileoverview Adapter for Grok models, providing integration with Grok's
 ...
 * 
 * The GrokAdapter provides specialized integration with Grok's models,
...
class GrokAdapter extends EventEmitter {
  ...
  constructor(options) {
    ...
    if (!options) throw new Error('Options are required for GrokAdapter');
    if (!options.logger) throw new Error('Logger is required for GrokAdapter');
    if (!options.configService) throw new Error('ConfigService is required for GrokAdapter');
    if (!options.performanceMonitor) throw new Error('PerformanceMonitor is required for GrokAdapter');
    if (!options.securityManager) throw new Error('SecurityManager is required for GrokAdapter');
    if (!options.credentialManager) throw new Error('CredentialManager is required for GrokAdapter');
    if (!options.cacheManager) throw new Error('CacheManager is required for GrokAdapter');
    ...
    this.logger.info('GrokAdapter created');
  }
  ...
  async initialize() {
    try {
      this.logger.info('Initializing GrokAdapter');
      this.performanceMonitor.startTimer('grok.initialize');
      
      // Load configuration
      this.modelConfig = await this.configService.getModelConfig('grok');
      if (!this.modelConfig) {
        throw new Error('Failed to load configuration for Grok models');
      }
      ...
      // Initialize cache
      await this.cacheManager.initializeCache('grok', {
      ...
      this.performanceMonitor.stopTimer('grok.initialize');
      this.logger.info('GrokAdapter initialized successfully');
      
      return true;
    } catch (error) {
      this.performanceMonitor.stopTimer('grok.initialize');
      this.logger.error(`Failed to initialize GrokAdapter: ${error.message}`, { error });
      this.initialized = false;
      this.available = false;
  ...
  async _initializeApiClient() {
    this.logger.info('Initializing Grok API client');
    ...
    try {
      ...
      if (!credentials) {
        throw new Error('No valid credentials available for Grok API');
      }
      
      // Create API client
      const { GrokClient } = require(this.modelConfig.apiModule || './grok_client');
      ...
      this.logger.info('Grok API client initialized successfully');
    } catch (error) {
      this.logger.error(`Failed to initialize Grok API client: ${error.message}`, { error });
      throw error;
    }
  }
    ...
    setInterval(async () => {
      try {
        ...
        if (this.available !== isAvailable) {
          ...
          this.logger.info(`Grok API availability changed to: ${isAvailable}`);
        }
      } catch (error) {
        this.logger.error(`Health check failed for Grok API: ${error.message}`, { error });
      }
    }, healthCheckInterval);
  }
  ...
  getAdapterInfo() {
    return {
      ...
      version: '1.0.0',
      provider: 'Grok',
      description: 'Adapter for Grok models, providing integration with Grok\'s language models across all reasoning strategies',
  ...
  getCapabilities() {
    return {
      maxTokens: this.modelConfig?.contextSize || 32768, // Grok-2 default
      ...
      multimodal: this.modelConfig?.multimodal || true,
      supportsVision: this.modelConfig?.supportsVision || true,
  ...
  async isAvailable() {
    ...
    try {
      this.performanceMonitor.startTimer('grok.isAvailable');
      ...
      this.performanceMonitor.stopTimer('grok.isAvailable');
      return isAvailable;
    } catch (error) {
      this.performanceMonitor.stopTimer('grok.isAvailable');
      this.logger.error(`Error checking Grok API availability: ${error.message}`, { error });
      return false;
    }
  }
  ...
  async generateCompletion(options) {
    if (!this.initialized) {
      throw new Error('GrokAdapter is not initialized');
    }
    ...
    try {
      this.performanceMonitor.startTimer('grok.generateCompletion');
      ...
      const cachedResult = await this.cacheManager.get('grok', cacheKey);
      
      if (cachedResult) {
        this.metrics.cacheHits++;
        this.performanceMonitor.stopTimer('grok.generateCompletion');
      ...
      // Cache result
      await this.cacheManager.set('grok', cacheKey, result);
      ...
      this.performanceMonitor.stopTimer('grok.generateCompletion');
      this.logger.error(`Error generating completion with Grok API: ${error.message}`, { error, options });
  ...
  async generateEmbeddings(options) {
    ...
    try {
      this.performanceMonitor.startTimer('grok.generateEmbeddings');
      this.metrics.totalRequests++;
      ...
      if (cachedResult) {
        this.metrics.cacheHits++;
        this.performanceMonitor.stopTimer('grok.generateEmbeddings');
      ...
      this.metrics.failedRequests++;
      this.performanceMonitor.stopTimer('grok.generateEmbeddings');
      this.logger.error(`Error generating embeddings with Grok API: ${error.message}`, { error, options });
  ...
  /**
   * Process a reasoning task using Grok models.
  ...
  async processTask(task) {
    ...
    try {
      this.performanceMonitor.startTimer(`grok.processTask.${task.type}`);
      this.metrics.totalRequests++;
      ...
      if (cachedResult) {
        this.metrics.cacheHits++;
        this.performanceMonitor.stopTimer(`grok.processTask.${task.type}`);
      ...
      this.metrics.failedRequests++;
      this.performanceMonitor.stopTimer(`grok.processTask.${task.type}`);
      this.logger.error(`Error processing task with Grok API: ${error.message}`, { error, task });
...
`;
  ...
  async shutdown() {
    try {
      this.logger.info('Shutting down GrokAdapter');
      ...
      this.logger.info('GrokAdapter shut down successfully');
      return true;
    } catch (error) {
      this.logger.error(`Failed to shut down GrokAdapter: ${error.message}`, { error });
      return false;
    }
  }
  ...
  async _getCredentials(userId) {
    try {
      ...
      if (userId) {
        const userCredentials = await this.credentialManager.getUserCredentials(userId, 'grok');
      ...
      const globalCredentials = await this.credentialManager.getGlobalCredentials('grok');
      if (globalCredentials && globalCredentials.apiKey) {
        return globalCredentials;
      }
      
      this.logger.warn('No valid credentials found for Grok API');
      return null;
    } catch (error) {
      this.logger.error(`Error getting credentials for Grok API: ${error.message}`, { error });
      return null;
    }
  }
  ...
  _estimateCost(model, usage) {
    try {
      ...
      const pricing = {
        // Completion models
        'grok-2': {
          input: 0.000007, // $7 per million input tokens
          output: 0.000021 // $21 per million output tokens
        },
        'grok-1.5': {
      ...
      // Default to Grok-2 pricing if model not found
