...
class ReasoningEngine extends EventEmitter {
  ...
  async cleanupTasks(maxAgeMs) {
    ...
    try {
      ...
      if (this.logger && cleanedCount > 0) {
        this.logger.debug(`Cleaned up ${cleanedCount} old reasoning tasks`);
      }
      
      return cleanedCount;
    } catch (error) {
      if (this.logger) {
        this.logger.error(`Failed to clean up tasks: ${error.message}`, { error: error.stack });
      }
      throw error;
    } finally {
      if (this.performanceMonitor && timerId) {
  ...
  /**
   * Gets statistics about the reasoning engine.
   * 
   * @returns {Promise<Object>} - Engine statistics
   */
  async getStatistics() {
    this.ensureInitialized();
    ...
    let timerId;
    if (this.performanceMonitor) {
      timerId = this.performanceMonitor.startTimer("reasoningEngine_getStatistics");
    }
    ...
    try {
      ...
      const strategyStats = await this.reasoningStrategyManager.getStatistics();
      ...
      if (this.performanceOptimizer) {
        statistics.performance = await this.performanceOptimizer.getStatistics();
      }
      
      if (this.uncertaintyHandler) {
        statistics.uncertainty = await this.uncertaintyHandler.getStatistics();
      }
