 ...
 * 
 * @module core/metrics/tentacle_metrics/TentacleMetricsManagerTest
 */
...
describe('TentacleMetricsManager', () => {
  ...
  describe('anomaly detection', () => {
    it('should emit anomaly events when threshold is exceeded', () => {
      // Create a real metrics collector for this test
      const realMetricsCollector = new MetricsCollector();
      
      // Create tentacle metrics with lower threshold for testing
      const tm = new TentacleMetricsManager({
      ...
      // Set up spy for anomaly events
      const anomalySpy = sinon.spy();
      tm.subscribeToAnomalyAlerts(anomalySpy);
      
      // Register tentacle
      tm.registerTentacle('test-tentacle', 'test');
      
      // Record normal values to establish baseline
      for (let i = 0; i < 20; i++) {
        tm.recordTentacleMetric('test-tentacle', 'operation.duration', 100 + Math.random() * 10);
      }
      
      // Record anomalous value
      tm.recordTentacleMetric('test-tentacle', 'operation.duration', 500);
      
      // Check that anomaly event was emitted
      assert.ok(anomalySpy.called);
      assert.strictEqual(anomalySpy.args[0][0].tentacleId, 'test-tentacle');
      assert.strictEqual(anomalySpy.args[0][0].metricName, 'tentacle.operation.duration');
      assert.strictEqual(anomalySpy.args[0][0].value, 500);
